...
export class User {
  ...
  async lookupInvoice(payment_hash: string) {
    try {
  ...
  async getUserInvoices(limit?: string) {
    ...
    for (let item of range) {
      ...
      if (invoice.payment_route) {
        ...
        if (
          ...
          invoice.payment_route.total_amt_msat / 1000 !== +invoice.payment_route.total_amt
        ) {
          ...
          invoice.value =
            ...
            Math.max(
              // @ts-ignore
              parseInt(invoice.payment_route.total_amt_msat / 1000),
